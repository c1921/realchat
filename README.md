...
主动消息并不是单独的一套聊天系统，而是复用主发送链路。

流程如下：

1. `ProactiveMessagingController` 按配置定时检查
2. 到达触发时间后构造一个 catalyst
3. catalyst 的含义是：“距离上次对话过去了多久，请角色主动发起话题或表达关心”
4. `ChatViewModel` 调用同一个 `sendMessageInternal()`，但这次没有用户输入
5. 最终只落一条助手消息
